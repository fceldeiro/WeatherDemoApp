...
typedef void (^MWWeatherServiceParseCompletitionBlock)(MWWeatherData* weatherData,NSError* error);

@interface MWWeatherService ()
@property (nonatomic, weak) id <MWWeatherServiceDelegate> delegate;

@property (nonatomic,copy) NSString * cityName;

- (instancetype)initWithDelegate:(id <MWWeatherServiceDelegate> )delegate NS_DESIGNATED_INITIALIZER;
@end

@implementation MWWeatherService
...
+ (MWNetworkingConfigurationDataJSON *)configurationBasic
{
	...
	configuration.path = @"/data/2.5/weather";
...
- (instancetype)initWithDelegate:(id <MWWeatherServiceDelegate> )delegate
{
	if (self = [super init]) {
...
- (instancetype)initWithCityName:(NSString *)cityName countryIdentifier:(NSString *)countryIdentifier andDelegate:(id <MWWeatherServiceDelegate> )delegate andUnitFormat:(MWUnitFormat) unitFormat
{
	if (self = [self initWithDelegate:delegate]) {
		MWNetworkingConfigurationDataJSON *basicConfig = [MWWeatherService configurationBasic];
...
- (void)parseWithNetworkingResponse:(MWNetworkingOperationResponse *)networkingResponse withCompletitionBlock:(MWWeatherServiceParseCompletitionBlock)completitionBlock{
    ...
    if (responseCodeInteger >= 400){
        ...
        MWWeatherData *weatherData = nil;
        if (dicResponse && [dicResponse isKindOfClass:[NSDictionary class]]) {
            weatherData = [[MWWeatherData alloc] initWithDictionary:dicResponse];
            weatherData.cityName = self.cityName;
        }
        
        completitionBlock(weatherData,nil);
    }
...
- (void)onServiceOperationFinished:(MWNetworkingOperation *)operation withResponse:(MWNetworkingOperationResponse *)response
{
    ...
    [self parseWithNetworkingResponse:response withCompletitionBlock:^(MWWeatherData *weatherData, NSError *error) {
        ...
        if (error){
            ...
            if ([self.delegate respondsToSelector:@selector(mwWeatherService:didFinishedWithWeatherData:)]) {
                [self.delegate mwWeatherService:self didFinishedWithWeatherData:weatherData];
            }
        }
    }];
...
- (void)onServiceOperationFailed:(MWNetworkingOperation *)operation withError:(NSError *)error
{
    ...
    if ([self.delegate respondsToSelector:@selector(mwWeatherService:didFailedWithError:)]) {
        [self.delegate mwWeatherService:self didFailedWithError:error];
    }
}
